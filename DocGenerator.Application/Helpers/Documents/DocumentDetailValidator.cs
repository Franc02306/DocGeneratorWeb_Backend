...
namespace DocGenerator.Application.Helpers.Documents
{
    public static class DocumentDetailValidator
    {
        ...
        #region VALIDACIONES DE ACTUALIZAR

        /// <summary>
        /// Ejecuta las validaciones para una lista de detalles en actualización.
        /// Valida líneas nuevas y existentes, omitiendo los detalles marcados como eliminados.
        /// </summary>
        public static List<string> ValidateUpdate(List<UpdateDocDetailRequest>? details)
        {
            var errors = new List<string>();
            ...
            for (int i = 0; i < details.Count; i++)
            {
                ...
                ValidateDeleteRulesUpdate(detail, errors, lineNumber);

                if (detail.IsDeleted)
                    continue;

                NormalizeUpdate(detail);
                ValidateRequiredFieldsUpdate(detail, errors, lineNumber);
                ValidateMaxLengthsUpdate(detail, errors, lineNumber);
                ValidateAmountsUpdate(detail, errors, lineNumber);
                ValidateTaxUpdate(detail, errors, lineNumber);
            }
        ...
        /// <summary>
        /// Normaliza textos del detalle en actualización.
        /// </summary>
        private static void NormalizeUpdate(UpdateDocDetailRequest detail)
        {
            detail.ItemCode = detail.ItemCode?.Trim().ToUpper();
            detail.TaxCode = detail.TaxCode?.Trim().ToUpper();
            detail.ProjectCode = detail.ProjectCode?.Trim().ToUpper();
        ...
        /// <summary>
        /// Valida reglas básicas para eliminación de detalles en actualización.
        /// </summary>
        private static void ValidateDeleteRulesUpdate(UpdateDocDetailRequest detail, List<string> errors, int lineNumber)
        {
            if (detail.Id == 0 && detail.IsDeleted)
                errors.Add($"Línea {lineNumber}: No se puede eliminar un detalle que no existe.");
        }

        /// <summary>
        /// Valida campos obligatorios del detalle en actualización.
        /// </summary>
        private static void ValidateRequiredFieldsUpdate(UpdateDocDetailRequest detail, List<string> errors, int lineNumber)
        {
            if (string.IsNullOrWhiteSpace(detail.ItemCode))
                errors.Add($"Línea {lineNumber}: El código de artículo es obligatorio.");
        ...
        /// <summary>
        /// Valida longitudes máximas permitidas para los campos del detalle en actualización.
        /// </summary>
        private static void ValidateMaxLengthsUpdate(UpdateDocDetailRequest detail, List<string> errors, int lineNumber)
        {
            ValidateMaxLength(detail.ItemCode, 50, "El código de artículo", errors, lineNumber);
            ValidateMaxLength(detail.TaxCode, 10, "El código de impuesto", errors, lineNumber);
            ValidateMaxLength(detail.ProjectCode, 20, "El proyecto", errors, lineNumber);
        ...
        /// <summary>
        /// Valida valores numéricos y consistencia entre cantidad, precio, subtotal y total en actualización.
        /// </summary>
        private static void ValidateAmountsUpdate(UpdateDocDetailRequest detail, List<string> errors, int lineNumber)
        {
            if (detail.Quantity.HasValue && detail.Quantity.Value <= 0)
                errors.Add($"Línea {lineNumber}: La cantidad debe ser mayor a 0.");
        ...
        /// <summary>
        /// Valida la coherencia del impuesto respecto al subtotal y total en actualización.
        /// </summary>
        private static void ValidateTaxUpdate(UpdateDocDetailRequest detail, List<string> errors, int lineNumber)
        {
            var hasTaxCode = !string.IsNullOrWhiteSpace(detail.TaxCode);
